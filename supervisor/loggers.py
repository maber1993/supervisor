...
class CustomerRotatingFileHandler(FileHandler):
    def __init__(self, filename, mode='ab', maxBytes=512 * 1024 * 1024,
                 backupCount=10, suffix=None, encoding=None):
        """
        Open the specified file and use it as the stream for logging.
        ...
        If maxBytes is zero, rollover never occurs.
        """
        self.mode = mode  # doesn't make sense otherwise!
...
def handle_file(logger, filename, fmt, rotating=False, maxbytes=0, backups=0, when=None):
    ...
    if when:
        if when == 'H':
            handler = CustomerRotatingFileHandler(filename, backupCount=backups, suffix='%Y-%m-%d_%H')
        elif when == 'M':
            handler = CustomerRotatingFileHandler(filename, backupCount=backups, suffix='%Y-%m-%d_%H-%M')
        else:
            handler = CustomerRotatingFileHandler(filename, backupCount=backups, suffix='%Y-%m-%d')
    else:
